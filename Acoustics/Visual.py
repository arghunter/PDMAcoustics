...
def gen_anim(rms_data,segments,subduration,testNum, interpScale,fov):
    ...
    im = ax.pcolormesh(azi_grid, ele_grid, rms_data_interp[:, :, 0], shading='auto', cmap='jet', vmin=0, vmax=1)
...
# def gen_anim(frame_queue, segments, subduration, testNum, interpScale, fov):
#     fig, ax = plt.subplots(figsize=(10, 7))
#     im = ax.imshow(np.zeros((segments, segments)), cmap="viridis", vmin=0, vmax=1000)  # Initialize empty frame
#     ax.set_title("RMS Power Distribution")
